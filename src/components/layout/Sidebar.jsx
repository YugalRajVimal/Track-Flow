...
import {
  ...
  RiCloseLine, RiShip2Line, RiTeamLine
} from 'react-icons/ri'
...
export default function Sidebar({ open, onClose }) {
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="mb-3">
          ...
          {NAV_ITEMS.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                isActive
                  ? `flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium ${sidebarColors.navActiveBg} ${sidebarColors.navActiveText} border border-brand-200 mb-1`
                  : `flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium ${sidebarColors.navInactiveText} hover:bg-gray-50 ${sidebarColors.navHoverText} transition-colors mb-1`
              }
            >
      ...
      <div className={`p-4 border-t ${sidebarColors.border} bg-white`}>
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium ${sidebarColors.logoutBtn} transition-all duration-200`}
        >
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className={`hidden lg:flex flex-col w-64 flex-shrink-0 ${sidebarColors.background} ${sidebarColors.border} border-r h-screen sticky top-0`}>
        {sidebarContent}
      </aside>
